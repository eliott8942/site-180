---
currentYear: 2026

# Banner
banner:
  title: "L'Association"
  content: >
    Nous sommes l'équipe de 180°C, l'association de promotion de la cuisine à l'UNIL et l'EPFL ! Nous avons pour mission de partager notre passion de la cuisine et la rendre simple, accessible et durable pour les étudiant.e.s.
    <br><br>
    Parmi nos projets : La **Spice League** : un top chef étudiant, le deuxième volume du livre de cuisine étudiante nommé **Fringale**, et le **Crieur**, notre carnet virtuel d'adresses étudiantes à Lausanne. <br><br>
    Nous nous organisons en 4 pôles: événementiel, création, sponsoring et communication. Plus d'informations sur chacun ci-dessous.

  image: "/images/association.png"
  button:
    enable: true
    label: "Nous rejoindre"
    link: "mailto:epfl180c@gmail.com"

# Poles
poles:
  - name: Comité
    description: Organe de direction de l'association
    theme: comity
    members:
      - name: Jens Nordgren
        role: Président
        type: chief
      - name: Maria Maqueo
        role: Vice-Présidente
        type: chief
      - name: Ilyas Bokhari Khomsi
        role: Responsable Création
      - name: Adèle Linette
        role: Trésorière
      - name: Joanne Jegou
        role: Responsable Sponsoring
      - name: Luca Deandrea
        role: Responsable Sponsoring
  - name: Communication
    description: Gestion de l'identité visuelle ainsi que les réseaux sociaux de l'association
    members:
      - name: Arij Ben Salah
        role: Membre Communication
      - name: Sarah Ihadadene
        role: Membre Communication
      - name: Tudor Vultur
        role: Membre Communication
  - name: Création
    description: Création de recettes
    members:
      - name: Ilyas Bokhari Khomsi
        role: Responsable Création
        type: chief
      - name: Vittorio Sandri
        role: Membre Création
      - name: Adélie Teissier Zimmermann
        role: Membre Création
      - name: Paul Nauche
        role: Membre Création
      - name: Hamish Starling
        role: Membre Création
      - name: Camille Begon
        role: Membre Création
      - name: Myriam Achour
        role: Membre Création
      - name: Carla Arnoult Mendoza
        role: Membre Création
      - name: Alise Michaud
        role: Membre Création
      - name: Selma Amziane
        role: Membre Création
      - name: Adrien Blaga
        role: Membre Création
  - name: Event
    description: Organisation des événements
    members:
      - name: Maria Maqueo
        role: Responsable Event
        type: chief
      - name: Ximena Rivero Pimentel
        role: Membre Event
      - name: Loris Cusumano
        role: Membre Event
      - name: Olympe de Trogoff
        role: Membre Event
      - name: Laura Andrea Quiroz Ortega
        role: Membre Event
      - name: Dalia Somekh
        role: Membre Event
      - name: Raphaël Caquant
        role: Membre Event
      - name: Amélie Coulon
        role: Membre Event
      - name: Yanya Yang
        role: Membre Event
      - name: Alexandre Huck
        role: Membre Event
      - name: Jeroen Kodde
        role: Membre Event
  - name: IT & Website
    description: Gestion du site web, du Crieur ainsi que du blog
    members:
      - name: Eliott Tornassat
        role: Membre IT & Website
  - name: Sponsoring
    description: Recherche et gestion des sponsors et des partenaires
    members:
      - name: Joanne Jegou
        role: Responsable Sponsoring
        type: chief
      - name: Luca Deandrea
        role: Responsable Sponsoring
        type: chief
      - name: Chadi Assaf
        role: Membre Sponsoring
---
